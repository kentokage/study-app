(() => {
  ...
  const APPLE_W = 186, APPLE_H = 147, STUMP_W = 105, STUMP_H = 89, GROUND_Y = 449;
  ...
  function reset(){ score=0; world=0; player={x:120,y:366,w:70,h:83,vy:0,onGround:true}; items=[{type:'apple',x:390,y:310,w:APPLE_W,h:APPLE_H},{type:'stump',x:570,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:755,y:275,w:APPLE_W,h:APPLE_H},{type:'stump',x:900,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:1090,y:330,w:APPLE_W,h:APPLE_H},{type:'stump',x:1270,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H}]; countEl.textContent=0; statusEl.textContent='Ready to run'; draw(); }
  ...
  function update(dt){const speed=260; if(keys.ArrowLeft)player.x-=speed*dt;if(keys.ArrowRight)player.x+=speed*dt;if(touchMoveTime>0){player.x+=touchMoveDirection*speed*dt;touchMoveTime-=dt}player.x=Math.max(20,Math.min(canvas.width-player.w-20,player.x));player.vy+=1500*dt;player.y+=player.vy*dt;if(player.y>=366){player.y=366;player.vy=0;player.onGround=true} world+=dt*(keys.ArrowRight||touchMoveDirection>0?90:45);items.forEach(o=>o.x-=dt*120);if(items[items.length-1].x<850){const type=Math.random()>.45?'stump':'apple';items.push({type,x:items[items.length-1].x+170+Math.random()*130,y:type==='stump'?GROUND_Y-STUMP_H:(Math.random()>.5?300:330),w:type==='stump'?STUMP_W:APPLE_W,h:type==='stump'?STUMP_H:APPLE_H})}const hitbox={x:player.x+18,y:player.y+14,w:player.w-30,h:player.h-18};for(const o of items){if(o.type==='apple'&&intersects(hitbox,o)){score++;o.got=true;countEl.textContent=score;if(score>best){best=score;bestEl.textContent=best;localStorage.setItem('chiikawa-best',best)}}if(o.type==='stump'&&!o.hit&&intersects(hitbox,{x:o.x+15,y:o.y+18,w:o.w-30,h:o.h-18})){o.hit=true;running=false;cancelAnimationFrame(raf);statusEl.textContent='Run ended';message.querySelector('strong').textContent=`You collected ${score} apple${score===1?'':'s'}!`;message.querySelector('span:not(.message-emoji)').textContent='Press Start to try another run';startButton.textContent='Run again';message.classList.remove('hidden');canvas.classList.add('shake');setTimeout(()=>canvas.classList.remove('shake'),300)}}items=items.filter(o=>!o.got&&o.x>-160)}
  function draw(){const W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#c7ebd8');g.addColorStop(1,'#f6e9c9');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);ctx.fillStyle='#a8d7ba';for(let i=0;i<8;i++){const x=((i*180-world*.28)%1100)-100;ctx.beginPath();ctx.arc(x,275,95,Math.PI,0);ctx.fill()}ctx.fillStyle='#8ccda7';ctx.fillRect(0,420,W,80);ctx.fillStyle='#76bb8d';for(let i=0;i<18;i++){const x=((i*75-world)%1000);ctx.fillRect(x,420,3,10)}items.forEach(o=>{if(o.x>W+20||o.x+o.w<0)return;if(o.type==='stump'&&stumpReady)ctx.drawImage(stumpSprite,stumpBounds.x,stumpBounds.y,stumpBounds.w,stumpBounds.h,o.x,o.y,o.w,o.h);else if(o.type!=='stump')ctx.drawImage(sprites[o.type],o.x,o.y,o.w,o.h)});ctx.save();ctx.translate(player.x,player.y);if(player.vy<0)ctx.rotate(-.08);ctx.drawImage(sprites.player,0,0,player.w,player.h);ctx.restore()}
  canvas.addEventListener('touchstart',e=>{if(e.cancelable)e.preventDefault();const touch=e.changedTouches[0],rect=canvas.getBoundingClientRect(),localX=touch.clientX-rect.left;touchStart={x:touch.clientX,y:touch.clientY,side:localX<rect.width*.35?-1:localX>rect.width*.65?1:0};if(running&&touchStart.side){touchMoveDirection=touchStart.side;touchMoveTime=Infinity}},{passive:false});
  canvas.addEventListener('touchend',e=>{if(e.cancelable)e.preventDefault();if(!touchStart)return;const touch=e.changedTouches[0],dx=touch.clientX-touchStart.x,dy=touch.clientY-touchStart.y,side=touchStart.side;touchStart=null;if(side){touchMoveDirection=0;touchMoveTime=0;return}if(!running)return;if(Math.abs(dx)<28&&Math.abs(dy)<28){jump();return}if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>28){touchMoveDirection=dx>0?1:-1;touchMoveTime=.55}},{passive:false});
