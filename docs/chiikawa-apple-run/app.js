(() => {
  ...
  function start(){ if(running)return; running=true; message.classList.add('hidden'); music.play().catch(()=>{}); if(isPortrait&&isTouchDevice&&gameArea.requestFullscreen&&!document.fullscreenElement)gameArea.requestFullscreen().catch(()=>{}); last=performance.now(); raf=requestAnimationFrame(loop); }
  ...
  musicButton.addEventListener('click',()=>{music.muted=!music.muted;music.volume=MUSIC_VOLUME;musicButton.textContent=music.muted?'🔇':'🔊';musicButton.setAttribute('aria-label',music.muted?'Unmute music':'Mute music')});
