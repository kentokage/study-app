(() => {
  ...
  const keys = {}; const easterEgg = ['ArrowUp','ArrowUp','ArrowDown','ArrowDown','ArrowLeft','ArrowRight','ArrowLeft','ArrowRight','b','a','Enter']; let easterEggIndex = 0, mobileEggHoldTimer, cheatMode = localStorage.getItem('chiikawa-30-lives') === 'true'; let raf, last = 0, running = false, score = 0, lives = 3, best = Number(localStorage.getItem('chiikawa-best') || 0), world = 0, touchMoveDirection = 0, touchMoveTime = 0, touchStart = null, invulnerable = 0;
  let player, items;
  ...
  gameArea.addEventListener('touchstart',e=>{if(e.target.closest('.mobile-controls')||e.target.closest('button'))return;if(e.cancelable)e.preventDefault();const touch=e.changedTouches[0];touchStart={x:touch.clientX,y:touch.clientY};if(!running&&!message.classList.contains('hidden')){clearTimeout(mobileEggHoldTimer);mobileEggHoldTimer=setTimeout(()=>{if(!running&&!message.classList.contains('hidden'))activateCheat()},10000)}},{passive:false});
  gameArea.addEventListener('touchend',e=>{if(e.target.closest('.mobile-controls')||e.target.closest('button'))return;if(e.cancelable)e.preventDefault();clearTimeout(mobileEggHoldTimer);if(!running)return;if(!touchStart)return;const touch=e.changedTouches[0],dx=touch.clientX-touchStart.x,dy=touch.clientY-touchStart.y;touchStart=null;if(Math.abs(dx)<28&&Math.abs(dy)<28){jump();return}if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>28){touchMoveDirection=dx>0?1:-1;touchMoveTime=.55}},{passive:false});
  gameArea.addEventListener('touchcancel',e=>{clearTimeout(mobileEggHoldTimer);touchStart=null;touchMoveDirection=0;touchMoveTime=0},{passive:false});
