(() => {
  ...
  const startButton = document.getElementById('start-button'), fullscreenButton = document.getElementById('fullscreen-button');
  ...
  const APPLE_W = isPortrait ? 16 : 100, APPLE_H = isPortrait ? 13 : 79, STUMP_W = 105, STUMP_H = 89, GROUND_Y = isPortrait ? 560 : 449, APPLE_GROUND_Y = GROUND_Y - APPLE_H, PLAYER_W = isPortrait ? 90 : 70, PLAYER_H = isPortrait ? 107 : 83, PLAYER_GROUND_Y = GROUND_Y - PLAYER_H;
  ...
  function reset(){ score=0; world=0; player={x:isPortrait?55:120,y:PLAYER_GROUND_Y,w:PLAYER_W,h:PLAYER_H,vy:0,onGround:true}; items=isPortrait?[{type:'apple',x:190,y:APPLE_GROUND_Y,w:APPLE_W,h:APPLE_H},{type:'stump',x:365,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:630,y:APPLE_GROUND_Y-70,w:APPLE_W,h:APPLE_H},{type:'stump',x:810,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:1080,y:APPLE_GROUND_Y,w:APPLE_W,h:APPLE_H}]:[{type:'apple',x:390,y:APPLE_GROUND_Y,w:APPLE_W,h:APPLE_H},{type:'stump',x:570,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:755,y:APPLE_GROUND_Y-70,w:APPLE_W,h:APPLE_H},{type:'stump',x:900,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H},{type:'apple',x:1090,y:APPLE_GROUND_Y,w:APPLE_W,h:APPLE_H},{type:'stump',x:1270,y:GROUND_Y-STUMP_H,w:STUMP_W,h:STUMP_H}]; countEl.textContent=0; draw(); }
  function start(){ if(running)return; running=true; message.classList.add('hidden'); if(isPortrait&&isTouchDevice&&gameArea.requestFullscreen&&!document.fullscreenElement)gameArea.requestFullscreen().catch(()=>{}); last=performance.now(); raf=requestAnimationFrame(loop); }
  ...
  gameArea.addEventListener('touchend',e=>{if(e.target.closest('.mobile-controls')||e.target.closest('button'))return;if(e.cancelable)e.preventDefault();if(!running){if(!message.classList.contains('hidden')){reset();start()}return}if(!touchStart)return;const touch=e.changedTouches[0],dx=touch.clientX-touchStart.x,dy=touch.clientY-touchStart.y;touchStart=null;if(Math.abs(dx)<28&&Math.abs(dy)<28){jump();return}if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>28){touchMoveDirection=dx>0?1:-1;touchMoveTime=.55}},{passive:false});
  ...
  window.addEventListener('keydown',e=>{if(['ArrowUp','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault();keys[e.key]=true;if(e.key==='ArrowUp'||e.key===' ')jump()});window.addEventListener('keyup',e=>keys[e.key]=false);startButton.addEventListener('click',()=>{reset();start()});reset();
})();
